...
- Demo-first adoption is a runtime invariant: before a curated or external source is selected or implemented, inspect its official demo gallery and every official demo reasonably relevant to the assigned product role, then record the adaptation or rejection. Not viewing relevant demos requires a narrow evidence-backed hard exemption. For 3D, apply the suitability gate before a package or demo becomes a serious candidate; a failed gate ends 3D scouting cleanly. Preserve this behavior in the vendor-neutral core rather than a host-only adapter.
...
- Distinct multi-page work must differ in visible information structure, not only theme tokens, fonts, imagery, labels, or swapped 3D shapes. Preserve the route-structure comparison. Any adopted 3D must pass scene-specific subject/camera/material/light/motion and geometry-integrity gates: no accidental intersection, z-fighting, clipping, animation collisions, or finished models that read as stock primitives pushed together. Do not add a new framework or heavyweight validation layer.
