...
- Distinct multi-page work must separate the reusable collection shell from each work's visible architecture. Navigation, filters, minimal external metadata, tokens, and invisible infrastructure may be shared; preview interiors, composition, media rhythm, task/reading journey, motion grammar, and ending may not collapse into one numbered/labelled/CTA template. Preserve the route signature, preview-only contact sheet, and text-hidden silhouette comparison. Any adopted 3D must pass scene-specific subject/camera/material/light/motion and geometry-integrity gates: no accidental intersection, z-fighting, clipping, animation collisions, or finished models that read as stock primitives pushed together. Do not add a new framework or heavyweight validation layer.
