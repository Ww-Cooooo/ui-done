...
- UI 框架或组件系统
- 动画库和平滑滚动
- Three.js 等 3D / Canvas 工具
- 有真实数据时使用的图表或可视化工具
- 图标、图片、字体等资源系统
- 加载、体积和运行性能工具

具体选什么不固定，可以是 React、Motion、Lenis、Three.js、Chart.js，也可以是更适合现有项目的方案。它不会因为用户没说库名，就只交付一套最基础的页面。这对刚接触前端，或者会写一点代码、但不是专门做前端的人尤其有用：你负责说清页面给谁用、要做什么、希望是什么感觉，UI Done 负责把容易漏掉的前端选型补上。
...
- 做完整页面或较大改版时，即使用户没有点名，也会主动为 UI、动画、滚动、3D、图表、图标和性能工具选择主方案并实际使用。
