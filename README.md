...
展厅不再只用十套视觉皮肤证明“会做漂亮页面”。六页直接做成能完成工作的产品界面：训练复盘、航天告警处置、门店补货、日程与习惯、创意审阅、建筑项目协作；每页都有不同的使用者、核心动作、信息结构和“筛选或选择 → 查看 → 操作 → 状态反馈”闭环。另四页保留海岸编辑、当代艺术、赛博娱乐和当代新中式的表现力。十页先画十张互不重复的黑白线框，再分别实现成全屏赛道、轨道驾驶舱、连续小票、周历矩阵、可拖胶片桌、无限蓝图、全屏章节、单一展室、传送门状态机与横向长卷。作品页不共用可见页头、返回位置、左右栏、卡片阵列、详情容器、底部控制、进度形态、主轴、媒体比例、滚动方向或手机折叠方式；共用的只剩路由、状态、无障碍、故障回退和构建等看不见的基础设施。
...
| 页面 | 类型、使用者与目标 | 独有可见架构 | 手机端怎样变化 | 3D 决定 |
| --- | --- | --- | --- | --- |
| Velocity Works | 工作产品 · 跑步教练 · 选择训练、比较负荷、写复盘并完成 | 全屏比赛回放场；数据贴在跑道与影像上，场次是一条贴底时间轨 | 保留全屏赛道，时间轨横向移动，复盘原位升起 | 不用：趋势图和记录更清楚 |
| Orbital Grid | 工作产品 · 值班控制员 · 核对告警、装填指令并确认处置 | 地球居中的径向驾驶舱；告警沿轨道分布，指令从底部弧形控制台升起 | 驾驶舱纵向拉长，告警仍围绕核心，不改成列表页 | 使用：轨道关系与遮挡需要空间 |
| Corner Goods | 工作产品 · 社区店长 · 找到低库存、填补货并盖章 | 一张从店铺机器吐出的连续小票；搜索、库存、表单和盖章都在纸卷上 | 纸卷占满窄屏，继续向下推进同一张小票 | 不用：库存关系由清单和图表表达 |
| Still Day | 工作产品 · 个人使用者 · 新增安排、完成习惯并更新节奏 | 七天周历矩阵；日期、事件与习惯直接落在格网，不另开详情板 | 保留宽周历并横向浏览，不折成通用卡片堆 | 不用：时间与进度已经准确 |
| Atelier Noir | 工作产品 · 创意负责人 · 拖拽选片、比较版本、批注或批准 | 暗房灯桌上的重叠胶片；比较尺横跨画面，决定写在斜放的批注纸带 | 胶片继续重叠，批注纸带收紧，审批留在纸带内 | 不用：原图细节比装饰模型重要 |
| Grid 01 | 工作产品 · 建筑团队 · 选择坐标问题、写记录并推进状态 | 可自由定位的无限蓝图；问题是坐标热点，详情锚定在对应位置 | 保留画布和坐标关系，详情移到下部但不变成侧栏 | 使用：曲面壳体与协作路径帮助定位 |
| North Tide | 表现页面 · 自然读者 · 滚动让海岸摄影与文字逐章显影 | 每章都是一块 100vh 全幅海岸；没有共用页头或内容栏 | 仍逐屏阅读，文案固定在本章安全区 | 使用：连续海面就是内容 |
| Red Form | 表现页面 · 展览观众 · 在四个展室间切换作品与策展文字 | 单一展览舞台；作品、展签与房间编号在同一空间硬切，控制落在地面 | 舞台竖向收窄，展签落到下方，房间标记仍在地面 | 使用：连续雕塑就是展品 |
| Neon Rift | 表现页面 · 活动观众 · 经过 GATE、ARENA、INPUT 三阶段 | 全屏传送门状态机；内容随阶段换位，底部只有一块缺口式控制台 | 维持单屏光场与底部缺口，不改成长页 | 使用：粒子隧道承担入口空间 |
| Shanshui Now | 表现页面 · 文化读者 · 用滚轮、拖动或方向键展开三段长卷 | 一卷连续横向手卷；段落、题签、摄影和印章沿卷轴推进 | 仍横向换卷，正文落在独立宣纸区，摄影保留右侧 | 不用：摄影与排版已经充分 |
...
| 多页面项目 | 容易复制同一套首屏、卡片、图表和表单再换颜色 | 先把共用的集合外壳与每个作品的可见结构分开；用户要求“每页完全不同”时进入零可见结构复用模式，连页头、返回位置、主轴、详情容器、控制区、滚动方向和手机重排都逐页独立 |
| 组件、动效、可视化或素材来源 | 知道库名才可能去找示例 | 每次新页面或较大视觉改版都主动完成 MotionSites、React Bits、Uiverse、Anime.js、Aceternity 五源勘察，再记录采用、改造、只取思路或拒绝 |
...
遇到多页面站点、作品集或展示项目，它还会先把外层画廊和内部作品分开，再做逐页规划：每一页是什么产品、给谁用、核心动作是什么、怎样开场、主要内容是什么形状、媒体和模块怎样推进、哪项状态会改变、如何结束、到了手机上任务怎样收拢、3D 是否真的合适。普通的多样化项目可以共用简洁的集合导航和外部元数据，但作品预览和页面构图不能共用一张可见模板；用户明确要求“每页布局都不一样”时，则进入更严格的零可见结构复用模式，连作品页头、返回位置、轨道、卡片组合、详情承载、控制停靠、进度、主轴、媒体比例、入场、滚动方向和手机折叠方式都不能跨页复用，只有看不见的 React 生命周期、路由、数据、无障碍、回退和构建可以共享。每一页还必须先定下自己的主要动效，不能用同一个淡入、抽屉或纵向长页给十页换色。验收会先画黑白线框，再逐对比较页面骨架、详情承载、滚动方式和动效触发，并隐藏标题、编号和说明只看作品轮廓；只要两页还能套进同一张线框图，就要返工。

如果用户要求全量重构，旧页面只作为回滚点和问题证据，不再自动成为新设计的拼装素材。UI Done 会保留真实内容、数据含义、URL、资产、无障碍和有效状态，但只有用户明确要求保留，或存在清楚的产品理由时，才延续旧构图。这样可以避免 Agent 因为“旧代码已经能跑”就被旧首屏、旧侧栏和旧模块顺序困住。

每次新页面或较大视觉改版都会做一次精简的“五源创意勘察”：从 [MotionSites](https://motionsites.ai/) 选一个相关方向，并在站点允许时复制一个公开可用的 Prompt 作为研究输入；从 [React Bits](https://reactbits.dev/) 选一个 React 组件看 Preview 和 Code；从 [Uiverse](https://uiverse.io/) 选一个局部 UI 看实际代码；用 [Anime.js](https://animejs.com/) 的相关 Demo/API 规划这个页面自己的 JavaScript 动画；再从 [Aceternity UI](https://ui.aceternity.com/) 选一个结构或组件看 Preview 和 Code。五个都要认真看并做决定，但不要求五套代码都装进页面：不合适、依赖冲突或许可不清楚时，明确拒绝或只吸收抽象思路。

其他认真比较或采用的组件库、动效库、可视化库或素材来源，也要先看与当前用途相关的官方 Demo，再按整站调性改造。不要求把无关展厅全部刷完，也不会把 Demo 的示例文字、假数据、远程图片、控制栏和展示外壳原样搬进产品。3D 会先过适配门槛；门槛不通过时不会为了“看过 Demo”继续浪费调研，也不会下载 WebGL 代码。查看 Prompt、Demo 或 Code 不等于获得复制许可；React Bits 当前是 MIT + Commons Clause，Aceternity 还混合免费与付费内容，都必须逐项核对实际依赖、用途和再分发条款。查看过程中也不能向第三方网站上传项目代码、内部截图、凭据或其他私密内容。
...
- 做多页面项目时，先把可共用的集合外壳与每个作品的可见结构分开，再逐页记录使用者、核心动作、开场构图、主要内容形态、媒体节奏、模块顺序、任务或阅读旅程、动效语法、结尾、状态、数据来源和手机端变化；编码前逐对检查页面骨架、详情/弹层承载、滚动模型和主要动效，不能用同一套编号、角色、大标题、说明栏、首屏、Drawer/Modal、媒体卡、3D 区、图表、表单、统一淡入和结尾换皮。
...
- 新页面或较大视觉改版先完成五源创意勘察；认真比较或采用其他组件库、动效库、可视化库或素材来源时，也先看与当前用途相关的官方 Demo。记录采用、改造、只取思路或拒绝的原因；除非存在明确的访问、安全、权限或网络硬限制，否则不能跳过。
- 说“用 UI Done”只授权当前描述的前端任务，不自动授权账号登录、发布或其他外部操作。新增依赖或修改锁文件前，Agent 要说明包名、用途和影响；当前请求没有包含这项授权，或宿主规则要求确认时，必须等用户同意再执行。
- 不为了凑技术栈编造内容。动画跟着内容和状态走；完整页面和较大改版默认必须有一个内容专属的主要动效，只有明确禁止、交付格式不兼容或实测性能/无障碍失败等硬理由才省略，安静的页面也可以用克制但真实的运动。3D 必须是特别合适的表达方式，过不了门槛就完全不用；图表只在真有数据时出现。默认能力没有合适主位，才收成贴合页面的小点缀。
...
| 工作 | 示例展厅选用 |
| --- | --- |
| 页面主体与交互 | React 19 + Ant Design 6；按钮、输入、选择、表单、标签、提示、页签、折叠与主题 token 都由同一组件系统负责；十个作品不再共用 Drawer/Modal 详情模板 |
| 动画 | Anime.js 4；负责首屏编排、滚动进入与状态连续性，降低动效时切成静态完成态 |
| 平滑滚动 | Lenis 1；三种设备共用同一滚动机制，并保留锚点、历史返回、键盘和 Ant Design 浮层；工作内容本身仍按电脑、平板和手机重新组织 |
| 条件式真实 3D/WebGL | Three.js + React Three Fiber；只在 North Tide 的连续海面、Red Form 的一体化雕塑、Orbital Grid 的地球与完整卫星、Neon Rift 的粒子隧道、Grid 01 的一体化曲面壳体与连续协作路径中按需加载。每个场景都检查多帧构图、穿模/闪面/裁切、结构连接和回退；其他页面不请求 3D 分块 |
