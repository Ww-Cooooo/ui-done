...
展厅不再只用十套视觉皮肤证明“会做漂亮页面”。六页直接做成能完成工作的产品界面：训练复盘、航天告警处置、门店补货、日程与习惯、创意审阅、建筑项目协作；每页都有不同的使用者、核心动作、信息结构和“筛选或选择 → 查看 → 操作 → 状态反馈”闭环。另四页保留海岸编辑、当代艺术、赛博娱乐和当代新中式的表现力。UI Done 现在进一步把“集合外壳”和“作品内部”分开：展厅可以共用筛选和简洁信息，作品本身不能再套同一组编号、角色、大标题、说明栏和按钮后只换颜色或图片。
...
| --- | --- | --- | --- | --- |
| Velocity Works | 工作产品 · 跑步教练 | 选择训练 → 比较负荷与触地 → 原位写复盘 → 标记完成 | 横向训练列先选场次，再进入回放与复盘 | 不用：趋势图和记录更清楚 |
| North Tide | 表现页面 · 自然读者 | 继续滚动，让摄影与文字按阈值显影 | 取消长钉住，改为自然章节阅读 | 使用：连续海面就是内容 |
| Red Form | 表现页面 · 展览观众 | 在四个展室间硬切作品、展签与策展文字 | 左侧房间索引保留，作品与展签上下接续 | 使用：连续雕塑就是展品 |
| Orbital Grid | 工作产品 · 值班控制员 | 选择告警 → 核对轨道与遥测 → 装填指令 → 确认处置 | 告警与指令优先，轨道降为任务上下文 | 使用：轨道关系与遮挡需要空间 |
| Corner Goods | 工作产品 · 社区店长 | 找到低库存 → 在连续小票填补货 → 盖章 → 库存更新 | 补货小票先出现，货架随后展开 | 不用：库存关系由清单和图表表达 |
| Still Day | 工作产品 · 个人使用者 | 选择日期 → 在时间河新增安排或完成习惯 → 节奏更新 | 单条时间河优先，习惯环移到后段 | 不用：时间与进度已经准确 |
| Atelier Noir | 工作产品 · 创意负责人 | 选择胶片 → 拖拽比较版本 → 批注或批准 → 就绪度更新 | 胶片横向选择，审阅台独立承接决定 | 不用：原图细节比装饰模型重要 |
| Neon Rift | 表现页面 · 活动观众 | 经过 GATE、ARENA、INPUT 三阶段进入活动世界 | 主光场在上、阶段内容和行动在下 | 使用：粒子隧道承担入口空间 |
| Shanshui Now | 表现页面 · 文化读者 | 用滚轮、拖动或方向键横向展开三段长卷 | 仍是横向分屏卷轴，每屏内部自然阅读 | 不用：摄影与排版已经充分 |
| Grid 01 | 工作产品 · 建筑团队 | 筛选专业 → 选择坐标热点 → 写记录 → 推进状态轨 | 锚定问题卡覆盖模型下部，状态轨横向浏览 | 使用：曲面壳体与协作路径帮助定位 |
...
| 多页面项目 | 容易复制同一套首屏、卡片、图表和表单再换颜色 | 先把共用的集合外壳与每个作品的可见结构分开；逐页规划开场构图、内容形态、媒体节奏、任务或阅读旅程、动效语法、结尾和手机端变化 |
...
遇到多页面站点、作品集或展示项目，它还会先把外层画廊和内部作品分开，再做逐页规划：每一页是什么产品、给谁用、核心动作是什么、怎样开场、主要内容是什么形状、媒体和模块怎样推进、哪项状态会改变、如何结束、到了手机上任务怎样收拢、3D 是否真的合适。导航、筛选、简洁的标题分类和底层代码可以共用；作品预览和页面构图不能共用一张可见模板。每一页还必须先定下自己的主要动效，不能用同一个淡入、抽屉或纵向长页给十页换色。验收会逐对比较页面骨架、详情承载、滚动方式和动效触发，并隐藏标题、编号和说明只看作品轮廓；只要仍像同一张卡片或同一条页面换皮，就要返工。
