...
前十个作品不是十套换皮模板。它们分别采用全屏赛道、轨道驾驶舱、连续小票、周历矩阵、可拖动胶片桌、无限蓝图、全屏章节、单一展览舞台、传送门状态机和横向长卷。第十一个动效试验页从上到下排列任务重排、滚动组装和路径缓动三个实验，也可以通过页首入口直接跳到其中一项。展厅里的预览卡片把三个效果串在一条斜向路径上；这张卡片的构图与实验详情页不同。
...
<table>
  ...
  <tr>
    <td width="50%" align="center">
      ...
      <strong>Atelier Noir</strong><br>工作型页面：创意负责人可以拖动选片、比较版本、填写批注并作出审批决定。<br><a href="https://ww-cooooo.github.io/ui-done/showcase/atelier-noir/">打开页面</a>
    </td>
    <td width="50%" align="center">
...
| --- | --- | --- | --- |
| Velocity Works | 数据和操作直接叠在全屏比赛回放与贴底时间轨上。 | 继续保留全屏赛道，时间轨改为横向浏览，复盘区从原位置升起。 | 不使用，因为趋势图和训练记录已经能清楚表达信息。 |
...
| Still Day | 日期、事件和习惯直接放进七天周历矩阵，不另外打开通用详情侧栏。 | 保留宽周历并允许横向浏览，不把它折成卡片堆。 | 不使用，因为时间和完成进度已经能够准确表达任务。 |
| Atelier Noir | 重叠胶片位于暗房灯桌上，比较尺跨过画面，批注和决定写在斜放纸带上。 | 胶片继续重叠，批注纸带收紧，审批操作仍然留在纸带内。 | 不使用，因为查看原图细节比增加装饰模型更重要。 |
...
| 动画 | 十个原作品仍由 Anime.js 4.5.0 负责首屏编排、滚动进入和状态连续性。总展厅中的动效试验预览和独立的 GSAP 动效试验页使用 GSAP 3.15.0 与 `@gsap/react` 2.1.2，分别演示任务换位、滚动组装和缓动路径。减弱动态模式会直接显示可用的静态完成状态。 |
...
| 独立 2D Canvas | Pts 0.12.9 分别承担速度轨迹、轨道参考、库存颗粒、计划涟漪、面料曲线、建筑网格和表达型页面的氛围效果。2D context 不可用时，正文仍然可以阅读和操作。 |
| 数据可视化 | AntV G2 5.4.8 把六个工作型页面中的演示记录整理为趋势、数量或就绪度图，也把总展厅的路由能力信息画成比较矩阵。所有图表都保留可读数字或表格作为等价内容。 |
...
十个原作品的现有效果没有为了演示新规则而整体迁移，它们仍然使用 Anime.js 4.5.0。仓库另外提供了 [`showcase/motion-lab/`](./showcase/motion-lab/) 作为 GSAP 小型试验页：它使用真实的本地任务状态与缓动样本，验证 UI Done 规定的 GSAP React 写法。GSAP 被拆成独立文件；总展厅需要它来播放第十一个展板的预览，独立试验页也会按需请求它，而前十个作品自己的路由不会因为这项新增内容加载 GSAP。
...
- GSAP 3.15.0 与 `@gsap/react` 2.1.2 由总展厅中的动效试验预览和独立动效试验页使用。两个发布包都把许可指向 GSAP 的 Standard License，但没有在包内附带独立许可文件，因此仓库保存了 2026 年 9 月 8 日核对的 [`licenses/GSAP-Standard-No-Charge-License.txt`](./licenses/GSAP-Standard-No-Charge-License.txt)，并把这份文字加入运行时许可汇总。使用或重新分发 GSAP 时，仍需重新查看当时有效的 [Standard License](https://gsap.com/community/standard-license/)，保留要求的权利声明，并检查产品是否属于或可能属于与 Webflow 竞争的无代码可视化网页动画制作工具。如果这一限制可能适用，在获得书面许可或换用兼容方案之前不能使用或分发相关实现。
