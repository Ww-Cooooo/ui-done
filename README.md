...
遇到多页面站点、作品集或展示项目，它还会先把外层画廊和内部作品分开，再做逐页规划：每一页是什么产品、给谁用、核心动作是什么、怎样开场、主要内容是什么形状、媒体和模块怎样推进、哪项状态会改变、如何结束、到了手机上任务怎样收拢、3D 是否真的合适。导航、筛选、简洁的标题分类和底层代码可以共用；作品预览和页面构图不能共用一张可见模板。验收时会隐藏标题、编号和说明，只看作品轮廓；如果仍然像同一张卡片换皮，就要返工。
...
- 做多页面项目时，先把可共用的集合外壳与每个作品的可见结构分开，再逐页记录使用者、核心动作、开场构图、主要内容形态、媒体节奏、模块顺序、任务或阅读旅程、动效语法、结尾、状态、数据来源和手机端变化；不能用同一套编号、角色、大标题、说明栏、首屏、媒体卡、3D 区、图表、表单和结尾换皮。
