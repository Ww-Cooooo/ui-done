...
| True 3D/WebGL | Three.js through React Three Fiber first, with `@react-three/drei` only for needed helpers | Which existing object, relationship, dataset, or motif benefits from real spatial depth, and how will the scene scale across phone/tablet/computer with a static fallback? |
...
| Typography | Curated-catalog gap: supplied or separately verified local/system/open fonts by language and role | Are glyph coverage, weights, license, size, and offline packaging known? |
