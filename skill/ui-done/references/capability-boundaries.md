...
| `$web-artifacts-builder` | React artifact scaffold and single-HTML bundling flow | Engineered for complex conversation artifacts; stack-specific and not a general repository workflow | Use only when that artifact format is the actual deliverable | Its scaffold, dependency bundle, or “always React” assumption |
...
- For substantial work, trigger a focused official-source scan across UI foundation, motion, scrolling, 3D/Canvas, visualization, icons/assets, and performance even when the user did not name tools.
...
| Artifact builder | Preserve a sound existing stack; for substantial greenfield work choose a compatible framework and full enhancement stack, while direct-open artifacts must bundle every selected layer locally. |
...
- Researching Astro, Lenis, post-processing, or abstraction layers can correctly end in rejection.
