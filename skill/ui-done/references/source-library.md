...
The status below is a time-bounded starting point. Before copying code or installing a package, use `$web-access` to recheck the official source, current API, package metadata, maintenance, exact license, framework compatibility, accessibility, and delivery constraints. Last verified: 2026-09-02.
...
| True 3D/WebGL | Three.js + React Three Fiber (`@react-three/fiber`) | **Official core plus React renderer and required first route** for real spatial scenes. Use `@react-three/drei` only for helpers the scene actually needs; do not substitute a 2D Canvas engine merely to avoid WebGL. |
...
| [Three.js](https://github.com/mrdoob/three.js) + [React Three Fiber](https://github.com/pmndrs/react-three-fiber) | True 3D/WebGL/WebGPU scenes in React | Native React renderer: `@react-three/fiber`; core: `three`; optional helpers: [`@react-three/drei`](https://github.com/pmndrs/drei) | Required first route for real 3D. On 2026-09-02 the official repositories were MIT, active, and showed about 115,013 and 31,844 GitHub stars respectively. Match R3F and React major versions, budget the GPU path, and retain a static/DOM fallback. |
...
- Use React Three Fiber over Three.js as the default React scene boundary for true 3D/WebGL. Reach for direct Three.js lifecycle code only when a low-level integration cannot be expressed cleanly through R3F. Treat `@react-three/drei` as an optional helper collection, not another renderer or a reason to add generic controls.
- Actively look for a bounded product-aligned 3D role in substantial work and scale quality for phone/tablet before omitting the layer. Treat 2D Canvas as a separate required category: Pts remains the creative-Canvas owner and Fabric.js the editable-Canvas owner; neither satisfies real 3D/WebGL coverage, and R3F does not satisfy 2D Canvas coverage.
