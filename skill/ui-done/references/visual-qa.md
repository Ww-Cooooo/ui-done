...
- For intentionally varied sets, the shared collection-shell boundary, each work's visible-architecture signature, and the contact-sheet plus text-hidden silhouette result. When zero-visible-reuse mode applied, include the desktop/phone topology comparison and confirm that shared code is invisible infrastructure only.
