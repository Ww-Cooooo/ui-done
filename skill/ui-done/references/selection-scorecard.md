...
- For an adopted 3D candidate, the subject is not inherently spatial, real 3D adds no unique communication value, or the team cannot finish coherent geometry, joints, proportions, materials, lighting, subject-specific motion, responsive performance, and a static/DOM fallback without accidental intersections, z-fighting, clipping, or animation collisions.
...
- For GSAP, the current Webflow/GreenSock Standard “No Charge” License does not cover the product or the product is or may be a no-code visual web-animation builder that competes with Webflow and written permission has not resolved the restriction. Do not mistake the MIT license of the optional official `greensock/gsap-skills` guidance repository for the runtime license.
...
Show this concise table before consequential installation. Reproduce all ten columns exactly; do not merge or rename them. For substantial work, begin with one proposed-adoption row for the React UI foundation, GSAP plus `@gsap/react` motion, Lenis plus ScrollTrigger scrolling, 2D Canvas, visualization, icons/assets, and performance, plus a separate true 3D/WebGL evaluation row. The React UI row must select a primary component system; Ant Design is the default for greenfield or ownerless work, and a no-adoption row is invalid. The motion row starts with GSAP adoption and can select another owner or no new owner only through the exact hard-rejection path in `SKILL.md`. The 3D row records the four suitability-gate results first and proposes Three.js/R3F plus a job only when all pass; `No — suitability gate failed` is valid with a specific reason. 2D Canvas remains a separate required row regardless of the 3D result. Visualization selects one owner, preferably AntV and otherwise ECharts when it fits better; use a no-adoption row only for the narrow proven absence of any authentic visualizable object or when another hard gate remains after alternatives and fallbacks.
...
- Animation: use GSAP plus `@gsap/react` as the default primary owner for presence, layout, coordinated state, timelines, and signature transitions on existing product surfaces. Inspect the official React guidance and a relevant Demo Hub or Showcase example before implementation. Keep CSS for isolated micro-states that do not compete with GSAP. Adopt Anime.js or another owner only through the exact GSAP hard-rejection or established-owner migration path. Do not expose library controls unless the user has a real manipulation task or an accessibility mechanism requires them.
- Scrolling: use Lenis through `lenis/react` as the default and only dedicated smooth-scroll mechanics owner and GSAP ScrollTrigger as the choreography owner. Do not add ScrollSmoother beside Lenis. Keep Lenis on computer, tablet, and phone when real-device-class checks pass; a native-only/no-adoption result requires an observed hard constraint, not convenience. Preserve forms, tables, Ant Design overlays, nested panels, anchors, keyboard paths, touch behavior, responsive changes, reduced-motion mode, and cleanup across navigation/remount.
- 3D: always evaluate, never force. Adopt only when all four conditions pass: a natural existing host, an inherently spatial subject, communication value unavailable from DOM/photo/video/SVG/2D Canvas, and a finishable modeling/performance/fallback budget. If any fails, record `3D not adopted: suitability gate failed: <specific reason>` and do not mount, request, initialize, or probe WebGL. If all pass, use Three.js through React Three Fiber as the preferred route; add `@react-three/drei` only for used helpers, inspect relevant official demos, provide a static fallback, and verify the full animation cycle has no accidental interpenetration, self-intersection, z-fighting, clipping, or collisions. Reject a finished subject that reads as unrelated stock primitives pushed together; require coherent silhouette, joins, proportion/detail hierarchy, material/light response, and subject-specific motion.
...
1. State the selected owner and shipped job for every default capability category, plus the attempted smaller role, alternatives, evidence, and hard-gate reason for any omission. State the 3D suitability result separately; a failed gate does not require a forced smaller role or hard exemption.
2. Attach the GSAP record for every new page or material redesign: official React guidance, exact relevant Demo Hub/Showcase example, observed capability, shipped job, selected plugins, planned adaptation, reduced-motion/fallback path, and current Standard License result; otherwise attach the exact hard-rejection evidence. Also attach the official-demo record for every other selected or seriously considered source. A 3D gate failure recorded before any serious candidate exists does not require a demo record.
3. Confirm package or copied-component name, official source, author/project, exact license, and compatible current usage from official evidence. Keep the GSAP runtime license separate from the MIT license of the optional official GSAP Agent Skills repository.
