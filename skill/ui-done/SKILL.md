...
3. Confirm whether the implementation is already React. Inspect routes, information architecture, representative screens/states, content, brand tokens, CSS strategy, component library, theme ownership, icons, fonts, animation ownership, requests/server data, client state, forms, data/charting, tests, build scripts, package manifest, lockfile, licenses, and distribution entry points. For an intentionally varied set, also identify every shared visible wrapper, header, navigation placement, overlay, metadata rail, hero, card array, section order, progress treatment, mobile stack, and preview component that may be imposing one composition on unrelated works. In zero-visible-reuse mode, remove those shared visible wrappers rather than styling around them.
4. Run the current build/tests when practical and capture baseline browser screenshots for redesigns. In a full visual overhaul, label those captures as rollback/defect evidence and do not carry their composition forward unless the preservation contract explicitly requires it.
...
- Shared animation helpers may own scope, cleanup, visibility, and reduced-motion behavior only. Each intentionally distinct page must author its own primary targets, trigger, timing relationship, spatial direction, and completion state. A universal opacity-and-translate reveal applied across the set cannot count as any page's primary motion and is a design defect when it becomes the dominant choreography.
...
Then use `$webapp-testing` or the available browser tooling. By default test one representative computer viewport, one tablet viewport, and one phone viewport. Add extra-wide, extra-narrow, orientation-specific, TV, kiosk, embedded, or other special targets only when the user or explicit delivery contract requires them. Also test representative pages and states; keyboard/focus; reduced motion; selected open-source fonts and representative glyphs; console/page errors; failed requests; horizontal overflow; fixed navigation clearance; Lenis wheel/touch/anchor/nested-overlay behavior; every triggered detail, menu, overlay, expanded region, and post-action state with computed foreground/background contrast; the collection-shell/individual-work boundary, pairwise interaction-topology and motion-signature comparison, and text-hidden silhouette comparison for intentionally varied sets; and, when 3D was adopted, R3F/WebGL performance, multiple animation phases, geometry integrity, and failure fallbacks. In zero-visible-reuse mode, also compare simplified desktop and phone wireframes with text, color, imagery, icons, and decoration removed; any repeated page edge, navigation locus, largest-mass partition, main axis, detail carrier, or mobile reflow is a failure even when component names and business roles differ. Capture at least two normal-motion states for each page's primary motion so “animation exists” is proven by an observable change rather than an import or selector. Confirm surfaces that rejected 3D do not mount or request WebGL. For offline delivery, open the actual artifact with its intended mechanism and confirm no required external network resources.
...
- For each materially redesigned page, the primary motion signature, observed start/end or before/after evidence, reduced-motion completion state, and any hard motion-omission reason. Also report the pairwise detail-carrier, scroll-model, and motion-topology result for intentionally varied sets.
